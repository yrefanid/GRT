...
// construct the list with the criteria
		criterion_consumption* r;
		criterion_consumption* r_inv=NULL;
		r=Operators[i].criteria;
		while (r!=NULL)
		{
			if (r_inv==NULL)
			{
				Inv_Operators[i].criteria=new criterion_consumption(r->object_ID,r->amount);
				r_inv=Inv_Operators[i].criteria;
			}
			else
			{
				r_inv->next=new criterion_consumption(r->object_ID,r->amount);
				r_inv=r_inv->next;
			}
			r=r->next;
		}
...
void create_inverted_action(action* act)
{
	...
	// check if there are criteria that can be produced...
	...
	if (nof_criteria>0)
	{
		...
		while (r!=NULL)
		{
			...
			if (r->object_ID>0)
				current_criterion=objects[r->object_ID].criterion;
			else
				current_criterion=objects[vars[-r->object_ID-1]].criterion;

			if (r->amount<0) 
				criteria[current_criterion].type=criteria[current_criterion].type | 2;
			else
				criteria[current_criterion].type=criteria[current_criterion].type | 1;
			r=r->next;
		}
	}
}
...
//	cout << h->f << endl;
	...
	bool found_better=false;		// Indicates if an old value-vector is better than the new one
	int i;
	...
	bool found_best=false;		// Indicates whether the new vector has min/max value 
								// either in a smaller best or in a greater best criterion.
	...
	if (old_distances!=NULL && enriched_no_used)	// WHAT IS THAT? THIS PROHIBITS FROM COMPUTING ALTERNATIVE DISTANCES
		return;										// IN CASE WHERE THE ENRICHED_NO_USED FLAG IS SET...

	// check whether the new distance has better evaluation value
	// than all other old distances...
	bool is_best=false;
	long new_best_value=vector_grade(v,1,-1,initial,true);

	if (h->subproblem_distances->best_distance==NULL)
		is_best=true;
	else
	{
		if (new_best_value<h->subproblem_distances->best_value)
		{
			is_best=true;
			h->subproblem_distances->best_distance->best_vector=false;
			// Check whether the old best vector "should be deleted".
			bool flag=false;
			int i=0;
			while(i<=nof_criteria && !flag) {
				flag=flag | h->subproblem_distances->best_distance->v_distance->mins[i]
						  | h->subproblem_distances->best_distance->v_distance->maxs[i];
				i++;
			}
			if (!flag)
				h->subproblem_distances->best_distance->should_be_deleted=true;
		}
	}
	...
	if (old_distances==NULL)
	{
		...
		for(i=0;i<=nof_criteria;i++)
		{
			...
			{
				////////////////////////////////////////////////////////////////////
				// Oysiastika ta increasing exoun min kai ta decreasing exoun max //
				////////////////////////////////////////////////////////////////////
	...
	{
		if (!old_distances->should_be_deleted || old_distances->best_vector)
		{
			// if an old vector is equal or better than the new vector...
			if (totally_better_equal(old_distances->v_distance, v))
				found_better=true;
			else
			{
			// if the new vector is better than the old vector
				if (totally_better_equal(v, old_distances->v_distance))
				{
					if (!old_distances->best_vector)
						old_distances->should_be_deleted=true;
					found_best=true;
					int	j;
					for (j=0;j<=nof_criteria;j++)
					{
						...
						if (old_distances->v_distance->maxs[j])
						{
							old_distances->v_distance->maxs[j]=false;
							v->maxs[j]=true;
						}	
					}
				}
				else if (!no_rdph) 
				{
					for(i=0;i<=nof_criteria;i++)
					{
						if (old_distances->v_distance->mins[i])
							if (is_min_i(v, old_distances->v_distance, i))
							{
						...
						if (old_distances->v_distance->maxs[i])
							if (is_max_i(v, old_distances->v_distance, i))
							{
								old_distances->v_distance->maxs[i]=false;
								v->maxs[i]=true;
								found_best=true;
								// check if the old_distance has more mins,
								// otherwise it sould be deleted...
					...
					// Here we search whether there are more mins || maxs in the 
					// current old vector (pointed by old_distances). 
					// However, in case where the new vector does not have a best dimension,
					// then there is no reason to check the old one...
					// So initially we set more_mins_maxs=!found_best   
					if (!old_distances->best_vector)
					{
						int j=0;
						bool more_mins_maxs=!found_best;
						while (j<=nof_criteria && !more_mins_maxs)
						{
							more_mins_maxs=old_distances->v_distance->mins[j] || old_distances->v_distance->maxs[j];
							j++;
						}
						if (!more_mins_maxs)
							old_distances->should_be_deleted=true;
	...
	if ((!found_better && (found_best || no_rdph)) || is_best)
	{
		// check whether the new vector has best combined value
		// but not best value in any individual dimension.
//		if (!found_best && is_best) {
//			cout << "*** bingo 34782740 ***" << endl;
//			abort();
//		}
...
//			cout << "Fact achieved " << nof_set_facts << " : " << f->f << endl;
		...
		*h->subproblem_distances->distances->v_distance=*v;
		...
		if (is_best) {
			h->subproblem_distances->best_distance=h->subproblem_distances->distances;
			h->subproblem_distances->best_value=new_best_value;
			h->subproblem_distances->distances->best_vector=true;
//			cout << "Best distance for " << h->f << "=" << *h->subproblem_distances->distances->v_distance 
		...
		linked_hash_entry* keep, linked_linked_distances* l_distances)
{
	...
	else	// if (remaining_precs!=NULL)
	{
		...
		*v = (*v) + (*inv_act->inverted_action_cost);
		...
		if (inv_act->best_distance==NULL)
			is_best=true;
		else
		{
			if (new_best_value<inv_act->best_value)
			{
				is_best=true;
				inv_act->best_distance->best_vector=false;
				// Check whether the old best vector "should be deleted".
				bool flag=false;
				int i=0;
				while(i<=nof_criteria && !flag) {
					flag=flag | inv_act->best_distance->v_distance->mins[i]
							  | inv_act->best_distance->v_distance->maxs[i];
					i++;
				}
				if (!flag)
					inv_act->best_distance->should_be_deleted=true;
			}
		}
		...
		while (old_distances!=NULL && !worse)
		{
			if (!old_distances->should_be_deleted || old_distances->best_vector)
			{
				...
				if (is_abs_min_equal(old_distances->v_distance, v))
					worse=true;
				else
				{
					if (is_abs_min(v, old_distances->v_distance))
					{
						found_best=true;
						for (i=0;i<=nof_criteria;i++)
						{
						...
						// If it has already been applied, there is no reason to delete it...
						if (old_distances->has_been_applied || old_distances->best_vector)
						{
							if (!old_distances->best_vector)
								old_distances->should_be_deleted=true;
					...
					else if (!no_rdph)	// check for absolute min...
					{
						...
						for (i=0;i<=nof_criteria;i++)
						{
							...
							// then there is no reason to check the old one...
							// So initially we set more_mins_maxs=!found_best   
							// Also in case where the old vector is still the best one, 
							// we cannot delete it.
							if (!old_distances->best_vector) {
								int j=0;
								bool more_mins_maxs=!found_best;
		...
		if (worse && found_best)
		{
			cout << "ERROR 8573957390" << endl;
			abort();
		}

		if ((!worse && (found_best || no_rdph) ) || is_best)
		{
...
//			}
			...
			if (is_best) {
				inv_act->best_distance=inv_act->distances;
				inv_act->best_value=new_best_value;
				inv_act->distances->best_vector=true;
			}
...
//		cout << *inverted_satisfied_head->inverted_step->step << endl;
		...
		{
//			cout << *action_distances->v_distance << endl;
			TEST_COUNTER3++;
			check_time();
			counter++;
...
//			cout << h->f << endl;
			if (h->subproblem_distances->distances==NULL)
			{
				...
				h->subproblem_distances->best_distance=h->subproblem_distances->distances;
				h->subproblem_distances->distances->best_vector=true;
				h->subproblem_distances->best_value=vector_grade(h->subproblem_distances->best_distance->v_distance,1,-1,initial,true);
			}
		}
	}
}
...
vector* get_inverted_action_cost(inverted_action* inv_action)
{
	vector* v=new vector();
	v->elements[0]=1;
	criterion_consumption* r=Operators[inv_action->step->operator_id].criteria;
	while (r!=NULL)
	{
		...
		v->elements[objects[obj].criterion+1] += r->amount;
...
// it is not know which criteria are monotonic. 
