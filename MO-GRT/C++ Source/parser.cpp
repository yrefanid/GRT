...
bool load_consumption()
{
	criterion_consumption* Criteria=NULL;
	...
	while (ch=='(')
	{
		...
		if (!next_word(wrd))
		{
			cout << "Unexpected characters or end of file while loading operator's " << Operators[nof_operators].name << " criteria." << endl;
			cout << "Keyword 'amount' expected." << endl;
			return false;
		}	
		if (!strcmp2(wrd,"amount"))
		{
			cout << "Unexpected word: '" << wrd << "' while loading operator's " << Operators[nof_operators].name << " criteria." << endl;
			cout << "Keyword 'amount' expected." << endl;
			return false;
		}	
		...
		{
			cout << "Unexpected characters or end of file while loading operator's " << Operators[nof_operators].name << " criteria." << endl;
			cout << "Criterion name or variable expected." << endl;
			return false;
		}
		...
		else
		{
			...
			if (!found)
			{
				criteria[nof_criteria++].object_ID=Object_ID;
				if (display_messages)
					cout << "Object " << objects[Object_ID].name << " has been characterized as a criterion." << endl;
			}
		}
		...
		if (Criteria==NULL)
		{
			Operators[nof_operators].criteria=new criterion_consumption(Object_ID,Amount);
			Criteria=Operators[nof_operators].criteria;
		}
		else
		{
			Criteria->next=new criterion_consumption(Object_ID,Amount);
		...
		if (ch!=')')
		{
			cout << "Unexpected character: '" << ch << "' while loading operator's " << Operators[nof_operators].name << " criteria." << endl;
...
bool existing_criterion(int Criterion);

bool load_criteria()
{
	char ch;
	char wrd[max_sof_words+1];
	if (!next_number(&past_weight))
	{
		cout << "The weight of the past plan was expected" << endl;
		cout << "Loading problem file aborted" << endl;
		return false;
	}

	if (!next_number(&remaining_weight))
	{
		cout << "The weight of the remaining plan was expected" << endl;
		cout << "Loading problem file aborted" << endl;
		return false;
	}
	...
	while (ch=='(')
	{
		int current_criterion=-1;

		if (!next_word(wrd))
		{
			cout << "Criterion name expected." << endl;
			cout << "Loading problem file aborted." << endl;
			return false;
		}

		
		if (!strcmp2(wrd,"length"))
		{
			bool object_exists=false;
			int i=1;
			while (i<=nof_objects && !object_exists)
			{
				if (strcmp2(wrd, objects[i].name))
				{
					...
					if(display_messages)
						cout << "Object " << wrd << " already existed." << endl;
					if (!existing_criterion(i))
					{
						criteria[nof_criteria++].object_ID=i;
						current_criterion=nof_criteria-1;
					}
					else
					{
						int j;
						for(j=0;j<nof_criteria;j++)
							if (criteria[j].object_ID==i)
								current_criterion=j;
					}
				}
				i++;
			...
			{
				strcpy(objects[++nof_objects].name, wrd);
				criteria[nof_criteria++].object_ID=nof_objects;
				current_criterion=nof_criteria-1;
				if(display_messages)
					cout << "Criterion object " << wrd << " created." << endl;
			}
		}
		else
			current_criterion=-1;

		int criterion_weight;
		if (!next_number(&criterion_weight))
		{
			cout << "The weight of criterion " << wrd << "was expected." << endl;
			cout << "Problem file loading aborted." << endl;
			return false;
		}
		
		if (current_criterion>=0)
			criteria[current_criterion].weight=criterion_weight;
		else
			length_weight=criterion_weight;
		
		ch=next_char();
		if (ch!='[' && ch!='(')
		{
			cout << "Left parenthesis or bracket expected (while loading criterion " << wrd << " scale)." << endl;
			cout << "Problem file loading aborted." << endl;
			return false;
		}
		...
		{
			if (ch=='[') 
				criteria[current_criterion].bounded_from=true;
			else
				criteria[current_criterion].bounded_from=false;
		}

		int scale_from;
		int scale_to;

		if (!next_number(&scale_from))
		{	
			cout << "Criterion " << wrd << " scale from: was expected." << endl;
			cout << "Problem file loading aborted." << endl;
			return false;
		}
		if (current_criterion>=0)
		{
			criteria[current_criterion].from=scale_from;
		}
		else
		{
			length_from=scale_from;
		}

		if (!next_number(&scale_to))
		{	
			cout << "Criterion " << wrd << " scale to: was expected." << endl;
			cout << "Problem file loading aborted." << endl;
			return false;
		}
		...
		if (current_criterion>=0)
		{
			criteria[current_criterion].to=scale_to;
		}
		else
		{
			length_to=scale_to;
		...
		{
			cout << "Scale from: is greater or equal to scale to: (while loading criterion " << wrd << ")." << endl;
			cout << "Problem file loading aborted." << endl;
			return false;
		}
		...
		if (ch!=']' && ch!=')')
		{
			cout << "Right parenthesis or bracket expected (while loading criterion " << wrd << " scale)." << endl;
			cout << "Problem file loading aborted." << endl;
			return false;
		}
		...
		{
			if (ch==']') 
				criteria[current_criterion].bounded_to=true;
			else
				criteria[current_criterion].bounded_to=false;
		}

		if (!next_word(wrd))
		{
			cout << "Keyword 'max' or 'min' expected (while loading criterion " ;
			if (current_criterion>=0) 
				cout << objects[criteria[current_criterion].object_ID].name << ")." << endl;
			else
				cout << "length" << ")." << endl;
			cout << "Problem file loading aborted." << endl;
			return false;
		...
		{
			if (current_criterion>=0)
				criteria[current_criterion].max_better=true;
		}
		else if (strcmp2(wrd, "min"))
		{
			if (current_criterion>=0)
				criteria[current_criterion].max_better=false;
		}
		else
		{
			cout << "Keyword 'max' or 'min' expected (while loading criterion " ;
		...
		if (ch!=')')
		{
			cout << "Left parenthesis expected (while loading criterion " ;
			if (current_criterion>=0)
				cout << objects[criteria[current_criterion].object_ID].name << ")." << endl;
			else
				cout << "length" << ")." << endl;
	...
	if (ch!=')')
	{
		cout << "ERROR: Unexpected character while loading criterion objects' names." << endl;
		cout << "Criterion name or right bracket expected." << endl;
		return false;
	}

	int i;
	for (i=0;i<nof_criteria;i++)
		objects[criteria[i].object_ID].criterion=i;
...
bool load_initial()
{
	...
	while (ch=='(')
	{
		...
		if (strcmp2(wrd,"amount"))
		{
			...
			if (r<0)
			{
				cout << "Wrong criterion name: '"<< wrd << "', while loading initial state."<< endl;
				initial2->nullify();
				return false;
			}
			...
			if (!next_number(&amount))
			{
				cout << "Cannot read criterion amount, while loading initial state."<< endl;
				initial2->nullify();
				return false;
			}
			initial2->state_vector->elements[r+1]=amount;
	...
	// NOTE THAT IN CASE WHERE FOR A SPECIFIC CRITERION
	// NON-INITIAL STATE AMOUNT IS SPECIFIED, THIS CRITERION IS CONSIDERED
	// TO HAVE A ZERO INITIAL AMOUNT...
//	if (nof_criteria>0) 
		for(i=0;i<=nof_criteria;i++)
			initial->state_vector->elements[i]=initial2->state_vector->elements[i];
...
bool existing_criterion(int Criterion)
{
	int i;
	for(i=0;i<nof_criteria;i++)
		if (criteria[i].object_ID==Criterion)
			return true;
	return false;
}
